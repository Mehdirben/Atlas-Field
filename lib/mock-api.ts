    ...
    if (isForest) {
        return {
            ...
            fire_risk_assessment: {
                ...
                dnbr_value: 0.05 + (Math.random() * 0.02),
                ndmi_value: 0.38 + (Math.random() * 0.1),
                msi_value: 0.85 + (Math.random() * 0.1),
...
export const mockApi = {
    ...
    runAnalysis: async (siteId: number, analysisType: string): Promise<Analysis> => {
        ...
        const interpretations: Record<string, string> = {
            ...
            EVI: isForest ? "Enhanced vegetation index for dense canopy shows optimal growth without saturation." : "Enhanced vegetation index shows strong biomass development, particularly in high-density areas.",
            NDWI: "Water stress analysis indicates optimal hydration levels. No immediate irrigation required.",
            NDRE: isForest ? "Red-edge chlorophyll analysis shows healthy nutrient uptake in the forest stand." : "Chlorophyll and nitrogen content analysis shows healthy nutrient uptake across the field.",
            NDMI: "Moisture levels are stable. No significant water stress detected in the central sectors.",
            MSI: "Moisture Stress Index confirms vegetation is well-hydrated with low risk of drying.",
            NBR: "Burn ratio analysis confirms no fire activity. Regeneration patterns are consistent with historical data.",
            dNBR: "Difference burn ratio confirms minimal impact from the most recent controlled activity.",
            SAVI: "Soil Adjusted Vegetation Index confirms high health in areas with varying canopy density.",
    ...
    getForestTrends: async (siteId: number): Promise<ForestTrends> => {
        ...
        return {
            analyses: dates.map((date, i) => ({
                ...
                dnbr: 0.05 + (Math.random() * 0.02),
                ndmi: 0.3 + (Math.random() * 0.1),
                msi: 0.8 + (Math.random() * 0.1),
                ndre: 0.45 + (Math.random() * 0.05),
                evi: 0.75 + (Math.random() * 0.1),
                savi: 0.7 + (Math.random() * 0.1),
